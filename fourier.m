...
%echantillons = [zeros(8000,size(XX,2));ZZrecons];
echantillons = ZZrecons;
echtest = echantillons - ones(size(echantillons))*(min(min(echantillons)));
echtest = echtest.*(3/max(max(echtest)));
for i = 1:size(echantillons,1)  
    echantillons2(i,:) = echantillons(size(echantillons,1)-i+1,:);
end ;

%echantillons = [echantillons2 ;echantillons];
figure;
%hold on;
for i = 1:size(XX,2)
	TG_inverse = [TG_inverse;real(ifft(echantillons(:,i)))];
    subplot(1,2,1)
    plot(  fftshift(echtest(:,i))   );
    subplot(1,2,2)
    plot(real(ifft( echtest(:,i)  )));
    pause;
end

H = TG_inverse(find(TG_inverse>=0.0001));

figure;
plot(TG_inverse)
freq2 = round(9004/(410112/44100));
% sound(H.^3,frequence_echantillonnage,taille_echantillon);
sound(H.^3,freq2,taille_echantillon);
